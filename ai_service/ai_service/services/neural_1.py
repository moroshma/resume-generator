...
class NeuralService:
    ...
    async def update_resume(self, current_data: List[Any], new_info: str):
        ...
        user_prompt_instruction = '\n\n---\nПроанализируй весь приведенный выше текст (текущие данные + инструкции) и верни ОБНОВЛЕННУЮ И ПОЛНУЮ информацию о кандидате СТРОГО в формате JSON-**списка** объектов: `[{"label":"название_поля","value":"обновленное_значение"}, ...]`. Сохраняй релевантные существующие поля, обновляй их или добавляй новые на основе инструкций. В ответе должен быть ТОЛЬКО этот JSON-список. сами label и values могут быть ТОЛЬКО строками'
